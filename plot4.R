...
plot(Global_active_power ~ DateTime, epc_data_subset, xaxt = "n", type = "l",
     xlab = "", ylab = "Global Active Power (kilowatts)")
axis.POSIXct(side=1, x=epc_data_subset$DateTime, format="%a", labels = TRUE)

plot(Voltage ~ DateTime, epc_data_subset, type = "l",
...
plot(Global_reactive_power ~ DateTime, epc_data_subset, xaxt = "n", type = "l",
     xlab = "datetime")
axis.POSIXct(side=1, x=epc_data_subset$DateTime, format="%a", labels = TRUE)

# Set device to png and plot to file
png(filename = paste0(epc_file_path_output, "plot4.png"), width = 480, 
    height = 480, units = "px")
par(mfrow = c(2,2))
